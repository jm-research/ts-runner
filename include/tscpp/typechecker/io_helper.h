#ifndef TSCPP_TYPECHECKER_IO_HELPER_H
#define TSCPP_TYPECHECKER_IO_HELPER_H
...
inline uint64_t readUint64(const vector<unsigned char>& bin,
                           unsigned int offset) {
...
inline uint32_t readUint32(const vector<unsigned char>& bin,
                           unsigned int offset) {
...
inline void writeUint32(vector<unsigned char>& bin, unsigned int offset,
                        uint32_t value) {
...
inline void writeInt32(vector<unsigned char>& bin, unsigned int offset,
                       int32_t value) {
  if (offset + 4 > bin.size())
    bin.resize(bin.size() + 4);
...
inline void writeUint64(vector<unsigned char>& bin, unsigned int offset,
                        uint64_t value) {
...
inline uint16_t readUint16(const vector<unsigned char>& bin,
                           unsigned int offset) {
...
inline void writeUint16(vector<unsigned char>& bin, unsigned int offset,
                        uint16_t value) {
  if (offset + 2 > bin.size())
...
inline string_view readStorage(const string_view& bin, const uint32_t offset) {
  ...
  return string_view(reinterpret_cast<const char*>(bin.data() + offset + 2),
                     size);
}
...
#endif  // TSCPP_TYPECHECKER_IO_HELPER_H