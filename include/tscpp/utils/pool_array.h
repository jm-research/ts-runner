#ifndef TSCPP_UTILS_POOL_ARRAY_H
#define TSCPP_UTILS_POOL_ARRAY_H
 ...
 */
template <typename T, size_t Items = 4096, size_t GCQueueSize = Items / 2,
...
class PoolArray {
 public:
  ...
  static_assert(Items >= GCQueueSize,
                "Items need to be bigger than GCQueueSize");
  ...
  struct Pool {
    ...
    explicit Pool(unsigned int slotSize)
        : slotSize(slotSize), gcQueueSize(ceil(GCQueueSize / slotSize)) {
  ...
  std::array<Pool, poolAmount> pools = {
      Pool(1),  Pool(2),   Pool(4),   Pool(8),   Pool(16),  Pool(32),
      Pool(64), Pool(128), Pool(256), Pool(512), Pool(1024)};
  ...
  Pool& getPool(unsigned int size) { return pools[poolIndex(size)]; }
  ...
  void clear() {
    ...
    for (auto&& pool : pools) {
      ...
      pool.gcQueued = 0;
      if (pool.firstBlock)
        initializeBlock(pool, pool.firstBlock);
    }
  }
 ...
 private:
  void allocateBlock(Pool& pool) {
    if (pool.currentBlock &&
        reinterpret_cast<slot_pointer>(pool.currentBlock)->header.next) {
      initializeBlock(
          pool, reinterpret_cast<slot_pointer>(pool.currentBlock)->header.next);
    } else {
      pool.blocks++;
      ...
      reinterpret_cast<slot_pointer>(newBlock)->header = {
          .prev = pool.currentBlock, .next = nullptr};
  ...
  void setNextBlock(Pool& pool, slot_pointer nextBlock) {
    if (pool.currentBlock)
      pool.currentBlock->header.next = nextBlock;
    if (!pool.firstBlock)
      pool.firstBlock = nextBlock;
  ...
  void initializeBlock(Pool& pool, slot_pointer nextBlock) {
    ...
    pool.lastSlot = reinterpret_cast<slot_pointer>(
...
#endif  // TSCPP_UTILS_POOL_ARRAY_H