#ifndef TSCPP_UTILS_POOL_BAND_H
#define TSCPP_UTILS_POOL_BAND_H
...
class PoolBand {
 public:
  ...
  T* allocate() {
    active++;
    auto size = sizeof(T);
    // std::cout<<"allocate "<<typeid(T).name()<<" "<<size<<" of blocksize " <<
    // blockSize << " of " <<end - currentSlot<<"\n";
 ...
 private:
  ...
  void allocateBlock() {
    if (currentBlock &&
      ...
      // one
      auto newBlock =
          reinterpret_cast<char*>(operator new(sizeof(Slot) + blockSize));
