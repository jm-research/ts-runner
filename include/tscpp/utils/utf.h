#ifndef TSCPP_UTILS_UTF_H
#define TSCPP_UTILS_UTF_H
...
inline bool isWhiteSpaceSingleLine(const CharCode& ch) {
  ...
  // specified by EcmaScript.
  return ch.code == CharacterCodes::space || ch.code == CharacterCodes::tab ||
         ...
         (ch.code >= CharacterCodes::enQuad &&
          ch.code <= CharacterCodes::zeroWidthSpace) ||
...
inline bool isLineBreak(const CharCode& ch) {
  ...
  // Only the characters in Table 3 are treated as line terminators. Other new
