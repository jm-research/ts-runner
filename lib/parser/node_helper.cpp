...
bool isAssignmentOperator(SyntaxKind token) {
  return token >= SyntaxKind::FirstAssignment &&
...
int getSyntacticModifierFlagsNoCache(node<Node> node) {
  ...
  if (node->flags & (int)NodeFlags::NestedNamespace ||
      (node->kind == SyntaxKind::Identifier &&
       to<Identifier>(node)->isInJSDocNamespace)) {
...
int getModifierFlagsWorker(node<Node> node, bool includeJSDoc,
  ...
  if (node->kind >= SyntaxKind::FirstToken &&
      node->kind <= SyntaxKind::LastToken) {
    return (int)ModifierFlags::None;
  }

  if (!(node->modifierFlagsCache & (int)ModifierFlags::HasComputedFlags)) {
    node->modifierFlagsCache = getSyntacticModifierFlagsNoCache(node) |
                               (int)ModifierFlags::HasComputedFlags;
  }

  // we don't support JSDoc
  //        if (includeJSDoc && !(node->modifierFlagsCache &
  //        (int)ModifierFlags::HasComputedJSDocModifiers) &&
  //        (alwaysIncludeJSDoc || isInJSFile(node)) && node->hasParent()) {
...
node<NodeUnion(PropertyName)> resolveNameToNode(const node<Node>& node) {
  ...
  throw runtime_error(fmt::format(
      "resolveNamToNode with kind {} no valid name property", (int)node->kind));
}
...
optionalNode<NodeArray> getTypeParameters(node<Node> node) {
  switch (node->kind) {
    ...
    default:
      throw runtime_error(
          fmt::format("node {} has no typeParameters", node->kind));
  }
}
...
string& getEscapedName(const node<Node>& node) {
  ...
  throw runtime_error(
      fmt::format("getEscapedName with kind {} no valid", (int)node->kind));
}
...
bool isThisIdentifier(optionalNode<Node> node) {
  if (!node)
    return false;
  return node->kind == SyntaxKind::Identifier &&
         identifierIsThisKeyword(to<Identifier>(node));
}
...
bool isSuperProperty(node<Node> node) {
  if (node->is<PropertyAccessExpression>())
    return to<PropertyAccessExpression>(node)->expression->kind ==
           SyntaxKind::SuperKeyword;
  if (node->is<ElementAccessExpression>())
    return to<ElementAccessExpression>(node)->expression->kind ==
           SyntaxKind::SuperKeyword;
  return false;
}
...
bool isBlock(const node<Node>& node) { return node->kind == SyntaxKind::Block; }
...
bool isJSDoc(const node<Node>& node) { return node->kind == SyntaxKind::JSDoc; }
