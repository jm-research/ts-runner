...
void asserts(bool v, std::string text) {
  if (!v)
    throw std::runtime_error("assert: " + text);
}

void fail(std::string text) { throw std::runtime_error("Fail: " + text); }
}  // namespace tscpp::Debug

namespace tscpp {
string substr(const string& str, int start, optional<int> len) {
  if (start <= 0 && len && len < 0)
    return "";
  if (!len || *len < 0 || len > str.size())
    *len = str.size();
  if (start < 0)
    start += str.length();
...
string replaceLeading(const string& text, const string& from,
                      const string& to) {
  if (0 == text.find(from)) {
    string str = text;
...
bool isTrue(optional<bool> b) { return b && *b; }

bool isFalse(optional<bool> b) { return !b || !*b; }
