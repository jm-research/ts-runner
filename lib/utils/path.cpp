...
bool isVolumeCharacter(const CharCode& charCode) {
  return (charCode.code >= CharacterCodes::a &&
          charCode.code <= CharacterCodes::z) ||
         (charCode.code >= CharacterCodes::A &&
          charCode.code <= CharacterCodes::Z);
}

int getFileUrlVolumeSeparatorEnd(const string& url, int start) {
  auto ch0 = charCodeAt(url, start);
  if (ch0.code == CharacterCodes::colon)
    return start + 1;
  if (ch0.code == CharacterCodes::percent &&
      charCodeAt(url, start + 1).code == CharacterCodes::_3) {
    auto ch2 = charCodeAt(url, start + 2);
    if (ch2.code == CharacterCodes::a || ch2.code == CharacterCodes::A)
      return start + 3;
  }
  return -1;
}
...
int getEncodedRootLength(const string& path) {
  ...
  // POSIX or UNC
  if (ch0.code == CharacterCodes::slash ||
      ch0.code == CharacterCodes::backslash) {
    if (charCodeAt(path, 1).code != ch0.code)
      return 1;  // POSIX: "/" (or non-normalized "\")

    auto p1 =
        path.find(ch0.code == CharacterCodes::slash ? directorySeparator
                                                    : altDirectorySeparator,
                  2);
    if (p1 < 0)
      return path.size();  // UNC: "//server" or "\\server"
  ...
  if (isVolumeCharacter(ch0) &&
      charCodeAt(path, 1).code == CharacterCodes::colon) {
    auto ch2 = charCodeAt(path, 2);
    if (ch2.code == CharacterCodes::slash ||
        ch2.code == CharacterCodes::backslash)
      return 3;  // DOS: "c:/" or "c:\"
    if (path.size() == 2)
  ...
  if (schemeEnd != string::npos) {
    ...
    if (authorityEnd != string::npos) {  // URL: "file:///", "file://server/",
                                         // "file://server/path"
      ...
      if (scheme == "file" && (authority == "" || authority == "localhost") &&
        ...
        auto volumeSeparatorEnd =
            getFileUrlVolumeSeparatorEnd(path, authorityEnd + 2);
        if (volumeSeparatorEnd != -1) {
          if (charCodeAt(path, volumeSeparatorEnd).code ==
              CharacterCodes::slash) {
            // URL: "file:///c:/", "file://localhost/c:/", "file:///c%3a/",
            // "file://localhost/c%3a/"
            return ~(volumeSeparatorEnd + 1);
          }
          if (volumeSeparatorEnd == path.size()) {
            // URL: "file:///c:", "file://localhost/c:", "file:///c$3a",
            // "file://localhost/c%3a" but not "file:///c:d" or "file:///c%3ad"
...
bool isAnyDirectorySeparator(const CharCode& charCode) {
  return charCode.code == CharacterCodes::slash ||
         charCode.code == CharacterCodes::backslash;
}

bool hasTrailingDirectorySeparator(const string& path) {
  return path.size() > 0 &&
         isAnyDirectorySeparator(charCodeAt(path, path.size() - 1));
}
...
vector<string> pathComponents(const string& path, int rootLength) {
  ...
  if (rest.size() && !lastOrUndefined(rest))
    rest.pop_back();
...
vector<string> reducePathComponents(const vector<string>& components) {
  ...
  for (int i = 1; i < components.size(); i++) {
    ...
    if (component == "..") {
      ...
      } else if (!reduced.empty() && !reduced[0].empty())
        continue;
    }
...
string normalizePath(string& _path) {
  ...
  //        auto normalized =
  //        getPathFromPathComponents(reducePathComponents(getPathComponents(path)));
  //        if (! normalized.empty() && hasTrailingDirectorySeparator(path))
  //        return ensureTrailingDirectorySeparator(normalized); return
  //        normalized;
}
}  // namespace tscpp
